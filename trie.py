# Source: https://github.com/AleksandrHovhannisyan/Python-Trie-Implementation
...
class PrefixTree:
    ...
    def __child_words_for(self, node, words, word_len=None):
        '''
        ...
        if node.is_word:
            ...
            elif word_len == len(node.text):
                words.append(node.text)
        for letter in node.children:
            self.__child_words_for(node.children[letter], words, word_len)

    def starts_with(self, prefix, word_len=None):
        '''
        ...
        self.__child_words_for(current, words, word_len)
        return words
